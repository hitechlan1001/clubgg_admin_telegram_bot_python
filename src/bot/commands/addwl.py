...
async def _addwl(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if not args:
            await update.message.reply_text("Usage: /addwl <club_id> <amount>\nExample: /addwl 492536 1000")
            return
        ...
        if len(args) >= 2:
            ...
            try:
                ...
                if update.effective_chat.type == "private":
                    await update.message.reply_text(f"❌ {e}\n\n💡 You can also specify a club ID: /addwl <club_id> <amount>")
                else:
                    await update.message.reply_text(f"❌ {e}")
                return
        ...
        check = can_manage_club(update, "addwl", int(club_id))
