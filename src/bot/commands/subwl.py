...
async def _subwl(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    try:
        ...
        if not args:
            await update.message.reply_text("Usage: /subwl <club_id> <amount>\nExample: /subwl 492536 1000")
            return
        ...
        if len(args) >= 2:
            ...
            try:
                ...
                if update.effective_chat.type == "private":
                    await update.message.reply_text(f"❌ {e}\n\n💡 You can also specify a club ID: /subwl <club_id> <amount>")
                else:
                    await update.message.reply_text(f"❌ {e}")
                return
        ...
        check = can_manage_club(update, "subwl", int(club_id))
